...
class mail_compose_message(osv.osv):
    ...
    def get_record_data(self, cr, uid, values, context=None):
        ...
        result = super(mail_compose_message, self).get_record_data(cr, uid, values, context=context)
        if context.get('option') == 'forward':
            parent_id = values.get('parent_id')
            for parent in self.pool.get('mail.message').browse(cr, uid, parent_id, context=context):
                active_tz = pytz.timezone(context.get("tz", "UTC")if context else "Europe/Rome")  # For CE time
                attendance_start = datetime.strptime(parent.create_date, DEFAULT_SERVER_DATETIME_FORMAT).replace(
                    tzinfo=pytz.utc
                ).astimezone(active_tz)
                next_attendance_date = datetime.strftime(attendance_start, "%a, %b %d, %Y at %H:%M %p")
                mail_date = datetime.strptime(
                    next_attendance_date, "%a, %b %d, %Y at %H:%M %p").strftime("%a, %b %d, %Y at %H:%M %p")
            ...
            re_prefix = _('Fwd:')
            if subject and not (subject.startswith('Fwd:') or subject.startswith(re_prefix)):
                subject = "%s %s" % (re_prefix, subject)
            result['subject'] = subject

            result['attachment_ids'] = attachment_ids

            for partner in self.pool.get('res.partner').browse(cr, uid, list(set(to_partner_ids)), context=context):
            ...
            body2 = "<br><br>---------- Forwarded message ----------<br><b>Subject: </b>"
