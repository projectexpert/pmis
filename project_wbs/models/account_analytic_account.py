...
class AccountAnalyticAccount(models.Model):
    ...
    def _child_deliverable_count(self):
        for account in self:
            account.child_deliverable_count = (
                account._child_count('deliverable')
            )
    ...
    def _child_work_package_count(self):
        for account in self:
            account.child_work_package_count = (
                account._child_count('work_package')
            )
    ...
    child_project_count = fields.Integer(
        "Projects",
        compute='_child_project_count',
        store=True
    )
    child_phase_count = fields.Integer(
        "Phases", compute='_child_phase_count', store=True
    )
    ...
    child_unclassified_count = fields.Integer(
        "Unclassified projects", compute='_child_unclassified_count', store=True
    )
