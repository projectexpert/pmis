    ...
    if inv.type in ('out_invoice', 'out_refund'):
        for i_line in inv.invoice_line:
            ...
            if (
                ...
                if not dacc:
                    catx = i_line.product_id.categ_id
                    dacc = (
                        catx.property_stock_account_output_categ and
                        catx.property_stock_account_output_categ.id
                    )
                else:
                    # = out_refund
                    ...
                    if not dacc:
                        dacc = (
                            catx.property_stock_account_input_categ and
                            catx.property_stock_account_input_categ.id
                        )
                # in both cases the credit account cacc will be the expense
                # account first check the product, if empty check the category
                cacc = (
                ...
                if not cacc:
                    cacc = (
                        catx.property_account_expense_categ and
                        catx.property_account_expense_categ.id
                    )
                if dacc and cacc:
                    for r in res:
                        anlx = i_line.account_analytic_id
                        if r['account_id'] == dacc:
                            r['account_analytic_id'] = anlx.id
