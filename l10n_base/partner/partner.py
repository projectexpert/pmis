...
class res_province(orm.Model):
    ...
    def name_get(self, cr, uid, ids, context=None):
        if not len(ids):
            return []
        res = []
        for province in self.read(cr, uid, ids, ['code', 'name'], context=context):
...
class res_partner(orm.Model):
    ...
    def check_category(self, cr, uid, ids, field_names, arg, context):
        ...
        for streetaddress in self.browse(cr, uid, ids):
            country_ids = country_obj.search(cr, uid, [('name', '=', streetaddress.country_id.name)])
            if country_ids:
                ...
                for country in countries:
                    for field_name in field_names:
                        ...
                        elif not result[streetaddress.id].get(field_name, False):
    ...
    _columns = {
        ...
        'enable_province': fields.function(
            check_category, string='Provinces?', type='boolean', readonly=True, method=True, multi=True),
        'enable_region': fields.function(
            check_category, string='Regions?', type='boolean', readonly=True, method=True, multi=True),
        'enable_state': fields.function(
            check_category, string='States?', type='boolean', readonly=True, method=True, multi=True, default=True),
    }
        ...
        if city:
            ...
            if city_ids:
                ...
                res = {'value': {
                    ...
                    city_row.region.country_id.id or False,
