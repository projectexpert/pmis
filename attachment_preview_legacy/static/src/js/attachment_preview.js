...
openerp.attachment_preview_legacy = function(instance)
{
    var _t = instance.web._t;
    openerp.attachment_preview_legacy.show_preview = function(
            attachment_id, attachment_url, attachment_extension,
            attachment_title)
    {
        var url = (window.location.origin || '') +
            '/attachment_preview_legacy/static/lib/ViewerJS/index.html#' +
            attachment_url.replace(window.location.origin, '') +
            '&title=' + encodeURIComponent(attachment_title) +
            '&ext=.' + encodeURIComponent(attachment_extension);
        window.open(url);
    };
    openerp.attachment_preview_legacy.can_preview = function(extension)
    {
    ...
    instance.web.Sidebar.include(
    {
        on_attachments_loaded: function(attachments)
        {
            ...
            this.$el.find('.oe-sidebar-attachment-preview')
                .click(this.on_attachment_preview_legacy);
            this.update_preview_buttons();
            return result;
        },
        on_attachment_preview_legacy: function(e)
        {
            e.preventDefault();
            e.stopPropagation();
            var self = this,
            ...
            {
                openerp.attachment_preview_legacy.show_preview(
                    attachment_id, attachment_url, attachment_extension,
                    attachment_title);
            }
            else
                ...
                .then(function(extensions)
                {
                    _(extensions).each(function(extension, id)
                    {
                        var $element = jQuery(
                            ...
                            + id + '"]');
                        if(openerp.attachment_preview_legacy.can_preview(extension))
                        {
