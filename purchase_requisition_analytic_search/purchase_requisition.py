...
class purchase_requisition(osv.osv):
    ...
    def _get_analytic_account_user_ids(
        self, cursor, user, ids, name, arg, context=None
    ):
        res = {}
        for purchase in self.browse(cursor, user, ids, context=context):
            ...
            for po_line in purchase.order_line:
                if po_line.account_analytic_id:
                    if po_line.account_analytic_id.user_id:
                        res[purchase.id].append(
                            po_line.account_analytic_id.user_id.id)
        return res
    ...
    def _search_analytic_accounts(self, cr, uid, obj, name, args, context):
        ...
        for field, operator, value in args:
            ...
            pr_line_ids = pr_line_obj.search(cr, uid, [('account_analytic_id',
                                                        operator, value)])
            order_ids = [
                pr_line.requisition_id and
                pr_line.requisition_id.id for
                pr_line in
                pr_line_obj.browse(cr, uid, pr_line_ids)
            ]
            res.append(('id', 'in', order_ids))
        return res

    def _search_analytic_account_user_ids(
        self, cr, uid, obj, name, args, context
    ):
        ...
        for field, operator, value in args:
            assert field == name
            pr_line_ids = pr_line_obj.search(
    ...
    _columns = {
        ...
        'account_analytic_user_ids': fields.function(
            _get_analytic_account_user_ids,
            type='many2many',
    ...
    def make_purchase_order(self, cr, uid, ids, partner_id, context=None):
        ...
        for requisition in self.browse(cr, uid, ids, context=context):
            po_req = po_obj.search(
                cr, uid, [('requisition_id', '=', requisition.id)],
                context=context
            )
            for po_id in po_req:
                pol_ids = pol_obj.search(
                    cr, uid, [('order_id', '=', po_id)]
                )
                for pol_id in pol_ids:
                    pol_brw = pol_obj.browse(cr, uid, pol_id)
                    raa_id = requisition.account_analytic_id
                    pol_obj.write(
                        cr, uid, [pol_brw.id],
                        {'account_analytic_id': raa_id.id},
                        context=context
                    )
        return res
